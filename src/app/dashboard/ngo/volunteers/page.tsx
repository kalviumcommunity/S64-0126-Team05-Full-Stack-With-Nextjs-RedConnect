        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">Manage and coordinate with your volunteer network.</p>
                </div>
                <div className="flex gap-3">
                    <button className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#16161f] border border-gray-200 dark:border-[#1f1f2e] rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1f1f2e] transition shadow-sm">
                        Export List
                    </button>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-[#1f1f2e]">
                            {volunteers.map((user) => (
                                <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-[#16161f] transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 px-2.5 py-1 rounded font-bold text-xs border border-red-100 dark:border-red-900/40 transition-colors">
                                            {user.type.replace("Type ", "")}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="space-y-1">
                                            <p className="text-gray-600 dark:text-gray-300 flex items-center gap-1.5"><Mail className="w-3 h-3" /> {user.email}</p>
                                            <p className="text-gray-400 dark:text-gray-500 text-xs flex items-center gap-1.5"><Phone className="w-3 h-3" /> {user.phone}</p>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-gray-500 dark:text-gray-400 font-medium">{user.joined}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1.5 w-fit ${user.status === 'Verified' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border border-green-100 dark:border-green-900/40' :
                                            'bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 border border-yellow-100 dark:border-yellow-900/40'
                                            } transition-colors`}>
                                    ...
                                    <td className="px-6 py-4">
                                        <button className="text-red-600 dark:text-red-500 hover:text-red-700 dark:hover:text-red-400 font-bold transition-colors">Message</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
